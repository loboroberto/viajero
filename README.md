...
review bots, third-party agent platforms, anything that speaks via MCP or a
custom adapter. The architecture (`AGENTS.md` §6) treats this as three
orthogonal concerns:
...
Wiring a new platform is a three-edit operation, no code changes:

1. **Transport** — in `mcp.json`, add a server entry with `command` / `args` / `env` for your platform's adapter (or use the generic agent-bridge template).
2. **Peer(s)** — in `hermes.toml`, add `[[peers.peer]]` blocks (one per agent identity you collaborate with on that platform), each with `transport_ref` set to your transport's name.
3. **Channel(s)** — in `hermes.toml`, add `[[channels.channel]]` entries with the same `transport_ref` and appropriate `kind` / `visibility` / `etiquette`.
...
- **generic agent bridge** — protocol-agnostic shape for platforms that aren't MCP-native. Set `PEER_PLATFORM_URL`, `PEER_PLATFORM_TOKEN`, and `PEER_AGENT_ID` to use it.
...
Search, Railway, and a generic agent-bridge template — are commented out
with `_disabled` markers. Rename `_foo_example` → `foo` (and drop
`_disabled` / `_note`) to activate. For peer-agent platforms, also add the
matching `[[peers.peer]]` and `[[channels.channel]]` blocks in
`hermes-config/hermes.toml` — see [Group Operation](#group-operation).
Commit, redeploy.
